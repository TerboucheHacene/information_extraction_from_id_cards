...
def separate_characters(imageB, imageG):
    ...
    while cmp < len(lines) - 1:
        ...
        cmp += 1
    characters = []
    characters.append(imageG[:, 0 : l[2]])
    cmp = 1
    while cmp < len(l) - 3:
        characters.append(imageG[:, l[cmp] : l[cmp + 3]])
        cmp += 2
...
def get_characters_from_image(image: np.array) -> List[List]:
    ...
    for i in range(0, 3):
        ...
        characters = [
            cv2.resize(c, (40, 60), interpolation=cv2.INTER_AREA) for c in characters
        ]
